...
int AddProcInBlackList(const char* buf)
{
	mm_segment_t fs;
	fs = get_fs();
	set_fs(KERNEL_DS);
	vfs_write(fileproc,buf,strlen(buf),&last_pos);
	printk("refresh %s\n",buf);
	set_fs(fs);
}

int DelProcInBlackList(const char* name)
{
	int start;
	int pos;
	int index=0;
	...
	mm_segment_t fs;
	size_t buf_len=4096;
	if((start=findProcInBlackList(name))!=NULL)
	{
		while(procfs_buffer[start]!='\n')
		{
			procfs_buffer[start]=' ';
			start++;
		}
		...
		if (buf == NULL)
 	   	   return -ENOMEM;
		while(procfs_buffer[start_buf]!='\0')
		{
			if(procfs_buffer[start_buf]!=' ')
			{
				buf[index]=procfs_buffer[start_buf];
				index++;
...
{
	initBlackList();
	//AddProcInBlackList("name/tz");
	//DelProcInBlackList("/have/fun");
	//AddProcInBlackList("namegdd");
	//findProcInBlackList("namegdd");
