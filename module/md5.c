...
static inline int getNextBlock(char *block)
{
    ...
    if ((res = vfs_read(file, block, MD5_BLOCK_SIZE, &len)) < MD5_BLOCK_SIZE)
    {
        char *plen;
        if (res < 0)  return -1;
        len *= 8;
...
int makeHash(const char *path_to_file, char *hash_result)
{
    ...
    fs = get_fs();
    set_fs(KERNEL_DS);
    file = filp_open(path_to_file, O_RDONLY, 0);
    if (IS_ERR(file))
    {
        return -1;
    }
    ...
    do
    {
        ...
        for (i = 0; i<64; i++)
        {
            ...
            stor = D;
            D = C; C = B;
            befor_rol = A + R + T[i] + X[j];
            B = B + ((befor_rol << S[i]) | (befor_rol >> (32-S[i])));
            A = stor;
        }
